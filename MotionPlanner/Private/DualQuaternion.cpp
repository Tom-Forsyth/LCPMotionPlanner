...
DualQuaternion::DualQuaternion(const Eigen::Quaternion<double>& argReal, const Eigen::Quaternion<double>& argDual) {
    this->real = argReal;
    this->dual = argDual;
}
...
DualQuaternion::DualQuaternion(const Eigen::Matrix4d& T) {
    ...
    Eigen::Quaternion<double> realVec{ R };
    Eigen::Vector4d pReverse = T.block(0, 3, 4, 1);
    pReverse(3) = 0;
    pReverse = 0.5 * pReverse;
    Eigen::Quaternion<double> realQuat{ pReverse };
    Eigen::Quaternion<double> dualQuat = realQuat * realVec;
    this->real = realVec;
    this->dual = dualQuat;
}
...
DualQuaternion DualQuaternion::operator+(const DualQuaternion& D) {
    ...
    Eigen::Quaternion<double> realQuat{ realVec };
    Eigen::Quaternion<double> dualQuat{ dualVec };
    DualQuaternion A{ realQuat, dualQuat };
    return A;
}
...
// Multiplication.
DualQuaternion DualQuaternion::operator*(const DualQuaternion& D) {
    Eigen::Quaternion<double> realQuat = this->real * D.real;
    Eigen::Quaternion<double> q1 = this->dual * D.real;
    Eigen::Quaternion<double> q2 = this->real * D.dual;
    Eigen::Quaternion<double> dualQuat = Eigen::Quaternion<double>{ q1.coeffs() + q2.coeffs() };
    DualQuaternion A{ realQuat, dualQuat };
    return A;
}
...
DualQuaternion DualQuaternion::conjugate() {
    Eigen::Quaternion<double> realQuat = this->real.conjugate();
    Eigen::Quaternion<double> dualQuat = this->dual.conjugate();
    DualQuaternion A{ realQuat, dualQuat };
    return A;
}
...
DualQuaternion DualQuaternion::pow(const double& n) {
    ...
    Eigen::Quaternion<double> realQuat{ D1.real, dualVec.at(0).real, dualVec.at(1).real, dualVec.at(2).real };
    Eigen::Quaternion<double> dualQuat{ D1.dual, dualVec.at(0).dual, dualVec.at(1).dual, dualVec.at(2).dual };
    DualQuaternion A{ realQuat, dualQuat };
    return A;
}
