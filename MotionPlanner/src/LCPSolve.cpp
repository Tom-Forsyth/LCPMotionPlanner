/*-----------------------------------------------
Author: Thomas Forsyth
Version: 1.0

Given M and q, solve the LCP for z and w.
...
LCP LCPSolve(MatrixXd M, VectorXd q) {
    ...
    int iter {0};
    bool solFound = checkSol(tableau);

    while ((!solFound) && (iter < maxIter)) {
        // Minimum ratio test to determine the next pivot row & pivot.
        int pivotRow = minRatioTest(tableau, pivotCol);
        pivot(tableau, pivotRow, pivotCol);

        // Check for secondary ray termination.
        int negTest {0};
        for (int i {0}; i < dim; i++)
            if (tableau.coeff(i, pivotCol) <= 0)
                negTest++;
