...
void SpatialManipulator::setObstacles(const std::vector<SphereObstacle> &obstacles) {
...
void SpatialManipulator::computeContacts() {
    ...
    obstacleDistances.reserve(this->dof);
    linkContactPoints.reserve(this->dof);
    contactNormals.reserve(this->dof);
    for (int i = 0; i < this->dof; i++) {
        // Link endpoints.
        ...
        for (int j = 0; j < nObstacles; j++) {
            ...
            double newDistance = ((centerPoint - q0) - h*(linkDisplacement)).norm() - this->obstacles.at(j).r - 0.06;
    ...
    for (int i = 0; i < dof; i++) {
        ...
        contactJacobians.push_back(jacobian);
    }
    this->contactJacobians = contactJacobians;
}
